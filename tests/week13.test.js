...
describe('Week 13: Regression & Reliability Tests', () => {
  ...
  it('Regression: App should not crash if activities data is missing', () => {
    ...
    assert.strictEqual(
      serverResponseCode,
      200,
      'System should return 200 OK, not 500 Error'
    );
    assert.deepStrictEqual(
      fallbackData,
      [],
      'System should generate an empty array if data is missing'
    );
  });
  ...
  it('Regression: Frontend should reject blank or negative workout times', () => {
    ...
    assert.strictEqual(
      validateInput(-10),
      false,
      'Negative minutes should be invalid'
    );
    assert.strictEqual(
      validateInput(30),
      true,
      'Positive minutes should be valid'
    );
  });
  ...
  it('Refactored Code: storage.js should save and retrieve data independently', () => {
    ...
    assert.deepStrictEqual(
      retrieved,
      testWorkout,
      'Storage layer should work perfectly isolated from the UI'
    );
  });
  ...
  it('Reliability: App should handle corrupted JSON in localStorage gracefully', () => {
    ...
    assert.deepStrictEqual(
      retrieved,
      [],
      'App should return empty array instead of crashing on bad JSON'
    );
  });
});
